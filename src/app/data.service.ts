...
export class DataService {
    ...
    cargarPersonas(){
        ...
        return this.httpClient.get('https://listado-personas-50414-default-rtdb.firebaseio.com/datos.json?auth='+ token);
    }

    //Guarda todo el arreglo de personas 
    guardarPersonas(personas: Persona[]) {
        const token = this.loginService.getIdToken();
        this.httpClient.put('https://listado-personas-50414-default-rtdb.firebaseio.com/datos.json?auth='+ token, personas)
            .subscribe(
                (response) => {
    ...
    modificarPersona(index:number, persona: Persona){
        let url: string;
        const token = this.loginService.getIdToken();
        url = 'https://listado-personas-50414-default-rtdb.firebaseio.com/' + '/datos/' + index + '.json?auth='+ token;
